...
const loginUser = async (email, password) => {
  try {
    ...
    if (user.uid !== ALLOWED_UID) {
      ...
      throw new Error("Bu kullanıcı ile giriş yapma yetkiniz yok!");
    }
...
onAuthStateChanged(auth, (user) => {
  if (user) {
    ...
    // UID kontrolü
    if (user.uid !== ALLOWED_UID) {
      console.log("Yetkisiz kullanıcı, çıkış yapılıyor...");
      signOut(auth);
    }
  } else {
...
const writeData = async (path, data) => {
  try {
    ...
    if (user.uid !== ALLOWED_UID) throw new Error("Bu kullanıcı ile işlem yapma yetkiniz yok!");
