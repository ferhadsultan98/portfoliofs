...
export default function AdminChatRight({ selectedUser }) {
  ...
  // Hazır cevapları inputa yazdıran fonksiyon
    ...
    <div className="adminChatRightContainer">
      {selectedUser ? (
        <>
          ...
          <div className="adminChatRightMessages">
            {messages.length > 0 ? (
              messages.map((message, index) =>
                message.sender === "user" ? (
                  <div key={index} className="message userMessage" title={formatTimestamp(message.timestamp)}>
                    <h6 style={{ fontWeight: "800" }}>
                      {message.text}
                    </h6>
                  </div>
                ) : (
                  <div key={index} className="message systemMessage" title={formatTimestamp(message.timestamp)}>
                    <h6 style={{ fontWeight: "800" }}>
                      {message.text}
                    </h6>
                  </div>
          ...
          {/* Hazır cevaplar için butonlar */}
          <div className="quickRepliesContainer" style={{ marginBottom: "50px" }}>
            <button
              onClick={() => handleQuickReply("Merhaba, nasıl yardımcı olabilirim?")}
              className="quickReplyButton"
            >
              Merhaba
            </button>
            <button
              onClick={() => handleQuickReply("Sorunuzu biraz daha detaylandırabilir misiniz?")}
              className="quickReplyButton"
            >
              Detay
            </button>
            <button
              onClick={() => handleQuickReply("Hemen kontrol ediyorum, bir saniye lütfen.")}
              className="quickReplyButton"
            >
              Kontrol
            </button>
            <button
              onClick={() => handleQuickReply("Teşekkürler, size en kısa sürede döneceğim.")}
              className="quickReplyButton"
            >
              Teşekkür
            </button>
            <button
              onClick={() => handleQuickReply("Sorununuzu çözmek için elimden geleni yapacağım.")}
              className="quickReplyButton"
            >
              Çözüm
            </button>
