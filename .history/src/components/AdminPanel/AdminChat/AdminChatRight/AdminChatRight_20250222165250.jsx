...
export default function AdminChatRight({ selectedUser }) {
  ...
  useEffect(() => {
    if (selectedUser) {
      ...
      const unsubscribe = onValue(messagesRef, (snapshot) => {
        if (snapshot.exists()) {
          const messagesData = Object.values(snapshot.val()); // Mesajları diziye çevir
  ...
  const handleSendMessage = () => {
    if (newMessage.trim() === "" || !selectedUser) return; // Boş mesaj veya kullanıcı yoksa gönderme

    const userEmailKey = selectedUser.email.replace(".", ",");
    const messagesRef = ref(database, `users/${userEmailKey}/messages`);

    const messageData = {
      sender: "system", // Admin tarafından gönderildiği için "system"
      text: newMessage,
      timestamp: Date.now(), // Mesajın zaman damgası
    };

    push(messagesRef, messageData) // Firebase'e mesajı ekle
      .then(() => {
