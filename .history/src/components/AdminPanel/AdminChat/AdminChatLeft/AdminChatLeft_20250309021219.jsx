...
const AdminChatLeft = ({ onUserSelect }) => {
      ...
      if (snapshot.exists()) {
        ...
        const userList = Object.keys(usersData)
          .map((key) => {
            ...
            return { 
              name: usersData[key].name || "Unknown User", // Add default value
              email: usersData[key].email || "", // Add default value
              lastMessageTime: lastMessage ? lastMessage.timestamp : null,
              lastMessageSender: lastMessage ? lastMessage.sender : null,
              userId: usersData[key].userId || "", // Add default value
            };
          })
          .sort((a, b) => {
  ...
  const filteredData = users.filter((user) => {
    ...
    return userName.toLowerCase().includes(nameSearch.toLowerCase());
  });
  ...
  const handleDragEnd = (result) => {
    ...
    const items = Array.from(users); // Use users instead of filteredData
          ...
          {(provided) => (
            ...
            >
              {filteredData.map((user, index) => (
                <Draggable
                  key={user.email || `user-${index}`} // Fallback key
                  draggableId={user.email || `user-${index}`}
                  index={index}
                >
                  {(provided) => (
                    <div
