...
const AdminChatLeft = ({ onUserSelect }) => {
  ...
  const formatDate = (timestamp) => {
    if (!timestamp || isNaN(timestamp)) return "No messages yet"; // Geçersiz timestamp kontrolü
    const current = new Date(timestamp);
    if (isNaN(current.getTime())) return "No messages yet"; // Tarih geçersizse
      ...
      if (snapshot.exists()) {
        ...
        const userList = Object.keys(usersData).map((key) => {
          const messages = usersData[key].messages
            ...
            : [];
          // En son mesajın timestamp'ini al, yoksa null döner
