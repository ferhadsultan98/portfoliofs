...
const AdminChatLeft = ({ onUserSelect }) => {
  ...
  const [activeTab, setActiveTab] = useState("all"); // Tab state ekledik
      ...
      if (snapshot.exists()) {
        ...
        const userList = Object.keys(usersData).map((key) => {
          ...
          return {
            ...
            lastMessageSender: lastMessage ? lastMessage.sender : null, // Kimin gönderdiğini ekledik
  ...
  // Filtrelenmiş verileri tab durumuna göre ayır
  const getFilteredData = () => {
    ...
    // "all" durumunda filtreleme zaten yukarıda yapılıyor
    return filtered;
  };
