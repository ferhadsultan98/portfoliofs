...
import { getAuth, onAuthStateChanged } from "firebase/auth"; // Authentication importu
import "./Projects.css";
...
const Projects = () => {
  ...
  const auth = getAuth(); // Authentication'ı başlatıyoruz

  const fetchData = async () => {
    try {
      if (user) { // Kullanıcı giriş yaptıysa veri çek
        ...
        setConsoleMessages(prev => [...prev, "User not authenticated"]); // Giriş yapmamış kullanıcı
      }
    } catch (error) {
  ...
  useEffect(() => {
    ...
    // Kullanıcı durumu değiştiğinde fetchData çağır
    fetchData();
