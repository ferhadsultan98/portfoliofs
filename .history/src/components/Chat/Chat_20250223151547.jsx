...
const Chat = () => {
  ...
  // Function to format timestamps (handles both string and Unix timestamp)
  const formatTimestamp = (timestamp) => {
    let date;
    if (typeof timestamp === "number") {
      // If timestamp is a Unix timestamp (e.g., 1740306186120)
              ...
              <div className="chatMessages">
                {messages.map((message, index) => (
                  <div key={index} className={`chatMessage ${message.sender}`}>
                    <p>{message.text}</p>
                    <i>{formatTimestamp(message.timestamp)}</i> {/* Format timestamp here */}
                  </div>
                ))}
              </div>
              <form
