...
const Chat = () => {
  ...
  useEffect(() => {
    if (messages.length > 0) {
      ...
      if (latestMessage.sender === "system" && !isMuted) { // Ses yalnızca isMuted false ise çalar
  ...
  const muteChat = () => {
    setIsMuted(!isMuted); // Ses susturma durumunu tersine çevir
    if (!isMuted && audioRef.current) {
      audioRef.current.pause(); // Ses çalıyorsa durdur
      console.log("Ses susturuldu");
    } else {
      console.log("Ses zaten susturulmuş veya tekrar açıldı");
    }
  };
