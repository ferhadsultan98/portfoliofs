...
const Chat = () => {
  ...
  // Create typewriter hooks for a fixed number of messages
  const typewriterHooks = useMemo(() => {
    return Array(100).fill(null).map(() => useTypewriter("")); // Pre-create 100 hooks (adjust as needed)
  }, []);

  // Manage displayed text for each message
  const displayedTexts = useMemo(() => {
    return messages.map((message, index) => {
      if (message.sender === "system") {
        typewriterHooks[index].reset(message.text); // Reset with new text
        return typewriterHooks[index];
      }
      return message.text;
    });
  }, [messages, typewriterHooks]);
  ...
  const handleUserSubmit = async (e) => {
    ...
    if (userInfo.name.trim() && userInfo.email.trim()) {
      ...
      if (snapshot.exists()) {
        ...
        initialMessages = [
          {
            text: `Hello, ${userInfo.name}! my name is Farhad Sultan, and I am a Frontend Developer. I specialize in creating user-friendly websites using HTML, CSS, and JavaScript. How can I assist you today?`,
            sender: "system",
