...
import useTypewriter from "react-typewriter-hook"; // Added typewriter import
...
const Chat = () => {
  ...
  // Added typewriter functionality
  const typewriterTexts = messages.map((message) =>
    message.sender === "system" ? useTypewriter(message.text, { speed: 50 }) : message.text
  );
            ...
            <div className="chatMessages">
              {messages.map((message, index) => (
                <div key={index} className={`chatMessage ${message.sender}`}>
                  <p>{typewriterTexts[index]}</p> {/* Updated to use typewriter */}
