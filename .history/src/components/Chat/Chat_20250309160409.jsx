...
const Chat = () => {
  ...
  const [playedMessages, setPlayedMessages] = useState(new Set()); // Yeni mesajların tekrar çalmasını önlemek için
  ...
  useEffect(() => {
    if (messages.length > 0) {
      ...
      const messageKey = `${latestMessage.timestamp}-${latestMessage.text}`; // Benzersiz bir anahtar oluştur

      // Yalnızca yeni bir sistem mesajı geldiğinde ve daha önce çalınmamışsa ses çal
      if (
        latestMessage.sender === "system" &&
        !isMuted &&
        !playedMessages.has(messageKey) &&
        lastAuthTime &&
        new Date(formatTimestamp(latestMessage.timestamp)) > new Date(lastAuthTime)
      ) {
        audioRef.current.play().catch((error) => {
          console.log("Ses çalma hatası:", error);
        });
        setPlayedMessages((prev) => new Set(prev).add(messageKey)); // Çalınan mesajı kaydet
  ...
  }, [messages, isChatVisible, isAuthenticated, isMuted, lastAuthTime]);
      ...
      <div className={`chatWindow ${isChatVisible ? "visible" : ""}`}>
        ...
        {!isAuthenticated ? (
          <form onSubmit={handleUserSubmit} className="userInfoForm">
            ...
            <button type="submit" className="chatLoginButton">
              LOGIN
              <i>
                <IoMdLogIn />
              </i>
