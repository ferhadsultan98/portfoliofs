...
const Chat = () => {
  ...
  const [currentTypingIndex, setCurrentTypingIndex] = useState(0); // Track which message is being typed
  ...
  const allowedDomains = ["gmail.com", "mail.ru", "yahoo.com", "hotmail.com", "outlook.com"];
  ...
  const handleEmailChange = (e) => {
    ...
    if (value.includes("@")) {
      ...
      if (!domainPart) {
        setSuggestedDomains(allowedDomains.map((domain) => `${prefix}@${domain}`));
      } else {
    ...
    if (isAuthenticated && userInfo.email) {
      ...
      unsubscribe = onValue(messagesRef, (snapshot) => {
        if (snapshot.exists()) {
          ...
          setCurrentTypingIndex(0); // Reset typing index when new messages load
        } else {
          setMessages([]);
          setIsAuthenticated(false);
          sessionStorage.removeItem("chatEmail");
  ...
  // Handle typewriter effect timing
  useEffect(() => {
    if (currentTypingIndex < messages.length) {
      const timer = setTimeout(() => {
        setCurrentTypingIndex((prev) => prev + 1);
      }, 1500); // Delay between messages (adjust as needed)
  ...
  const handleUserSubmit = async (e) => {
    ...
    if (userInfo.name.trim() && userInfo.email.trim()) {
      if (!validateEmail(userInfo.email)) {
        alert("Only gmail.com, mail.ru, yahoo.com, hotmail.com, and outlook.com are allowed!");
        return;
      }
      const emailKey = userInfo.email.replace(".", ",");
      const dbRef = ref(database);
        ...
        {!isAuthenticated ? (
          ...
          <>
            <div className="chatMessages" ref={chatMessagesRef}>
              {messages.slice(0, currentTypingIndex).map((message, index) => (
