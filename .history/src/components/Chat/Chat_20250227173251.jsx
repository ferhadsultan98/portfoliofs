...
const Chat = () => {
  ...
  const [playAudio, setPlayAudio] = useState(false); // Flag to indicate if audio should play
  const chatMessagesRef = useRef(null); // Auto-scroll ref
  const audioRef = useRef(new Audio(ChatNotificatioAudio)); // Audio ref
  ...
  // Set playAudio flag after 10 seconds
  useEffect(() => {
    const timer = setTimeout(() => {
      setPlayAudio(true); // After 10 seconds, allow audio to play on next interaction
    }, 10000); // 10 seconds delay

    return () => clearTimeout(timer); // Cleanup timer on unmount
  }, []);

  // Play audio when chat is toggled and 10 seconds have passed
  useEffect(() => {
    if (isChatVisible && playAudio) {
      audioRef.current.play().catch((error) => {
        console.log("Audio playback failed:", error);
      });
      setPlayAudio(false); // Reset flag so it only plays once
    }
  }, [isChatVisible, playAudio]);
